...
def scan1D(station, scanjob, location=None, liveplotwindow=None, plotparam='measured', verbose=1, extra_metadata=None):
    ...
    def myupdate():
        if liveplotwindow:
            t_start = time.time()
            liveplotwindow.update_plot()  # update()
            if verbose >= 2:
    ...
    for ix, x in enumerate(sweepvalues):
        ...
        if update_plot:
            if liveplotwindow:
                myupdate()
            pyqtgraph.mkQApp().processEvents()  # needed for the parameter viewer
    ...
    alldata.write(write_metadata=False) # Modified by JoKu at 28.07.2020.
...
def scan2D_poll_diff(station, scanjob, location=None, liveplotwindow=None, plotparam='measured', diff_dir=None, write_period=None,
           update_period=5, verbose=1, extra_metadata=None, demod_c = 3, poll_length = 0.003, digitizer = None, awg_daq = None):
    """

    This function is a version of the scan2D_poll function which takes one data point then changes the HDAWG user register 1.
    And then takes another data point and saves the differenc of the two. 
    ...
    Args:
        ...
        demod_c (int): 0-7 UHFLI demodulator channel
        poll_length (float): duration of poll acquisition, in seconds
        awg_daq: daq of the Zhinst HDAWG object
    ...
    for ix, x in enumerate(stepvalues):
        ...
        for iy, y in enumerate(sweepvalues):
            ...
            for d in range(2):
                ...
                awg_daq.setDouble('/dev8275/awgs/0/userregs/1', d) # So on the first passage the userreg 1 will have a value of 1
                                                                   # and on the second passage it will have a value of 0
                ...
                mean_r = np.mean(sample_r)  # Mean value of recorded data vector
                mean_fi = np.arctan2(mean_y,mean_x) * 180 / np.pi  # Calculating the angle value in degrees
                means.append({('demod{}_R'.format(demod_c+1)):mean_r, ('demod{}_phi'.format(demod_c+1)):mean_fi, 
                        ('demod{}_x'.format(demod_c+1)):mean_x, ('demod{}_y'.format(demod_c+1)):mean_y})
            #means = []
            for ii, p in enumerate(mparams):
                # Saving the demodulator data in the order defined in the scajob minstrument list
                k = str(p)[10:]
                datapoint[measure_names[ii]] = means[0][k] - means[1][k] # Substracting the point taken second from the one taken first
...
def get_sampling_frequency(instrument_handle):
    ...
    try:
        ...
        if isinstance(instrument_handle, qcodes.instrument_drivers.ZI.ZIUHFLI.ZIUHFLI):
            return instrument_handle.scope_samplingrate_float.get()
    except ImportError:
        pass
